...
export function CodeField({ label, help, error, success, verifying, length = 6, groupAfter, size, value, onChange, iconHref = 'icons.svg', className = '', ...rest }) {
  const cls = ['lds-field',
  ...
  const mark = (name) => React.createElement('svg', { className: 'lds-icon', 'aria-hidden': 'true' },
    React.createElement('use', { href: `${iconHref}#${name}` }));
